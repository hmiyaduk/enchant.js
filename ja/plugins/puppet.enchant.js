...
enchant.puppet.Puppet = enchant.Class.create(enchant.Sprite, {
    ...
    invoke: function(e) {
        var type = e.type;
        var handlers = this._behaviors[type];
        if (!handlers) {
            return;
        }
    ...
    registerBehavior: function(behavior) {
        for (var type in behavior) {
            if (!this._behaviors[type]) {
                this._behaviors[type] = [];
            }
            this._behaviors[type].push(behavior[type]);
        }
    },
    deleteBehavior: function(behavior) {
        var i;
        for (var type in behavior) {
            if (!this._behaviors[type]) {
                continue;
            }
            i = this._behavior[type].indexOf(behavior[type]);
            if (i !== -1) {
                this._behavior[type].splice(i, 1);
            }
        }
    },
    init: function() {},
...
enchant.puppet.Puppet.create = function(puppetName, option) {
    var definition = {
        initialize: function(x, y) {
            ...
            var behavior;
            for (var prop in this.behaviors) {
                if (typeof this.behaviors[prop] === 'string') {
                    behavior = enchant.puppet.Puppet.behaviors[this.behaviors[prop]];
                } else {
                    behavior = this.behaviors[prop];
                }
        ...
        /**
         * パペットからビヘイビアを削除する.
         * @param {[String|Object]} behaviors 削除したいビヘイビアの名前またはオブジェクト.
         */
    ...
    for (var i in option) {
        ...
        if (i === 'behavior' || i === 'ビヘイビア') {
            ...
            for (var j in behaviors) {
                ...
                definition.behaviors.push(behavior);
                /*
                for (var k in behavior) {
                    if (!definition['__' + k]) {
                        definition['__' + k] = [];
...
enchant.puppet.Puppet.behaviors = {
    ...
    standAlone: {
        sceneStart: function() {
            ...
            var startPins = [];
            if (!this.startPin) {
                startPins = [ [ theatre.width / 2, theatre.height / 2 ] ];
            } else {
