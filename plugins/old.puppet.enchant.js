...
enchant.Puppet = enchant.Class.create(enchant.Sprite, {
    initialize: function(x, y) {
        ...
        var path = (typeof this.filename === 'string') ? this.filename : 'chara1.png';
        ...
        this.init();
        stage.addChild(this);
        enchant.Puppet.instances.push(this);
    },
    // can override
    init: function() {
    },
...
enchant.Puppet.instances = [];

enchant.Puppet.sceneTouchendEventListener = [];
...
enchant.Puppet.create = function(puppetName, option) {
    var definition = {
        initialize: function(x, y) {
            ...
            if(listeners) for(var i in listeners) {
        ...
        die: function() {
            ...
            for(var i in puppets) {
                if(this == puppets[i]) puppets.splice(i, 1);
            }
        },
        onenterframe: function() {
            ...
            if((this.x < -320) || (this.x > 640) || (this.y < -320) || (this.y > 640)) this.die();

            if(!this.isHit) if(this.collision) {
                for(var i in this.collision) {
                    var puppets = window['___' + this.collision[i] + '_array'];
                    for(var j in puppets) {
                        if(this != puppets[j]) if(this.intersect(puppets[j])) {
        ...
        removeBehavior: function(behaviorName) {
            var behavior = enchant.Puppet.behaviors[behaviorName];
        ...
        if((i == 'behavior') || (i == 'ビヘイビア')) {
            ...
            for(var j in behaviors) {
                ...
                if(typeof behaviors[j] == 'string') {
                    behavior = enchant.Puppet.behaviors[behaviors[j]];
                } else {
    ...
    if(definition.__sceneTouchend) {
        var listeners = definition.__sceneTouchend;
        var that = this;
        for(var i in listeners) {
            listeners[i].puppetName = puppetName;
            //              enchant.Puppet.sceneTouchendEventListener.push(listeners[i]);
            enchant.Puppet.sceneTouchendEventListener.push(listeners[i]);
        }
    }
...
enchant.Puppet.clear = function() {
    ...
    enchant.Puppet.sceneEnterframeEventListener = [];
    enchant.Puppet.sceneStartEventListener = [];
    for(i in enchant.Puppet.instances) {
        var puppet = enchant.Puppet.instances[i];
    ...
    window.onload = function() {
        ...
        if(definition) {
            ...
            if (definition.assets) {
                theatre.preload(definition.assets);
            }
        }
        ...
        theatre.onload = function() {
            ...
            theatre.rootScene.addEventListener('touchend', function(e) {
                theatre.touchX = e.x;
                theatre.touchY = e.y;
                var listeners = Puppet.sceneTouchendEventListener;
                for(var i in listeners) {
                    var instances = window['___' + listeners[i].puppetName + '_array'];
...
var パペット = enchant.Puppet.create;
var パペット = enchant.Puppet;
var パペット作成 = function(name, definition) {
